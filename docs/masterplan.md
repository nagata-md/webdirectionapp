...
**2026-08-28 追記：ガントチャートの週末表示バグ修正、「並行作業人数」の概念を撤廃**

ユーザーから2件の指摘・要望を受けた。

1. **土日にガントバーが乗って見える不具合**：実際のプロジェクトのスクリーンショットで、8/30(日)・9/6(日)の列に色付きバーが乗っているのを発見。原因は`GanttChart.tsx`の待機区間（`waitSegments`）の終了日算出（`shiftCalendarDays`による暦日ベース計算）が営業日保証を持たず、`BarSegments`内の「最終日を濃い色で強調表示する」処理（区間の`end`プロパティをそのまま使用）が非稼働日チェックをしていなかったこと。`end`ではなく`getBusinessDayRuns()`が返す最後のrunの`end`（＝実際の最終稼働日）を使うように修正した。本体の営業日カウント（`shiftBusinessDays`）自体は正しく土日・祝日を除外しており、影響は待機区間の見た目の強調セルのみだった。
2. **「並行作業人数」という概念の撤廃**：ユーザーが「下層ページLの構成〜デザイン間の待機が想定（工数+チェックバック+2校作業+2校チェックバック=12日）より何日も長い」と指摘。調査の結果、マスタの`checkback`/`buffer`設定自体はほぼ想定通り（バッファ1日の差のみ）で、実際の原因は`並行作業人数`（デフォルト1＝直列）による他ページとのレーン待ち（`reserveLane()`）だった。ユーザーの最終判断：「並行作業人数という概念は不要。ガントチャートは想定日数のみで機械的に開始日を決め、実際に何人で並行して作業するかはシステムの外（社内運用）で解決する」。

   これに従い、`lib/schedule/laneAllocator.ts`（レーン管理ロジック）を削除し、`computeSchedule.ts`は全工程を「構成」工程と同じ「readyTimeに達し次第、他ページの状況に関わらず直ちに着手する」ロジックに統一した。あわせて型定義（`ComputeScheduleInput.parallelByPhase`、`ParallelByPhase`型）、マスタ設定画面の「並行作業人数（既定値）」列、保存処理（`master/actions.ts`）、プロジェクト複製時のコピー処理（`projects/actions.ts`）を削除し、`lib/schedule/__tests__/computeSchedule.test.ts`のレーン関連テスト（構成工程の特例テスト・レーン競合テスト）を新しい仕様に合わせて書き換えた。spec.md（§4.3・§4.4・§4.5・§4.10・§5・受け入れ基準）から該当記述を削除・更新済み。

   **未解消のまま残る既知のギャップ（今回は対応せず、記録のみ）**：前回の「優先度」撤廃時と同様、DBスキーマの列自体は破壊的変更を避けるため残置した（`master.default_parallel_by_phase`・`projects.parallel_by_phase`、いずれも`supabase/migrations/20260803015113_initial_schema.sql`）。アプリケーションコードからは一切参照しなくなったが、列自体は空の`jsonb`のまま残る。実害はないが、将来DBスキーマを整理する際にまとめてdropする候補として記録しておく。
   - `tsc --noEmit`・`eslint`・`vitest run`（全61件成功）を確認済み。実ブラウザでの最終確認はまだ行っていない。
3. **`docs/schedule.md`を新規作成**：スケジュール自動算出ロジックの仕様が複数箇所（spec §4.3・§4.5・§4.6、コードコメント）に分散し、かつ`並行作業人数`のように「仕様として書かれていたが実装は別の挙動をしていた」ギャップが繰り返し発生していたため、算出ロジックの正とする単一のリファレンスとして新設した。営業日・非稼働日の扱い、工程ごとの日数積み上げ式、2校期間の内訳、進行グループの起点計算を、実装（`computeSchedule.ts`）と突き合わせた最新の状態でまとめている。

> 次アクション候補：ユーザーによる実ブラウザでの動作確認待ち（週末列にバーが乗らなくなったか、下層ページLのようなケースで構成→デザインの遷移が想定日数どおりになるか）。

**2026-08-28 追記：ユーザーが実ブラウザで動作確認完了。「先方チェック待ち」と「社内バッファ待ち」を色分け**

ユーザーが本番環境で動作確認し「動作確認できました」との回答。続けて「先方チェック待ち」と「社内バッファ待ち」を色分けしたいとの要望を受けた。

- 色分けを正確に行うには、CMS構築・公開の2工程についてもチェックバックを独立区間として扱う必要があると判明した（従来はこの2工程だけ「チェックバック＋バッファ」を1つの暗黙の待機にまとめており、区別できる情報がそもそもなかった。かつ実際のマスタ値ではCMS構築=3日・公開=1日と無視できない値が入っている）。`computeSchedule.ts`のCMS構築・公開の分岐を、構成等4工程と同様に`pushSubSegment()`でチェックバック1の独立区間を積む方式に変更した（2校作業・チェックバック2は引き続き適用しない）。
- `app/globals.css`に`--color-phase-buffer`（ティール系、既存の週末グレー・チェックバックのピンクいずれとも被らない色）を追加。`GanttChart.tsx`・`ShareGantt.tsx`の`waitSegments`計算に`isBuffer`判定（直後のセグメントが次工程の制作＝`kind==="production"`かどうか）を追加し、社内バッファ待ちのみ新色で描画するようにした。凡例も「先方チェック待ち」「社内バッファ待ち」の2項目に分割。
- **外部共有閲覧画面への反映はユーザーに確認の上で対応**：「社内バッファ待ちという内訳をクライアントに開示してよいか」はビジネス判断が必要なため一度確認し、「見せてよい」との回答を得てから`ShareGantt.tsx`にも同じ色分けを適用した。
- **ついでに発見した重複バグも修正**：`ShareGantt.tsx`の`BarSegments`が、`GanttChart.tsx`で2026-08-28に修正したのと全く同じ「最終日強調セルが非稼働日チェックをしていない」バグを独立して抱えていた（コードが重複実装されているため）。同じ修正（`runs`の最後のrunの`end`を使う）を適用した。
- テスト：`computeSchedule.test.ts`にCMS構築のチェックバック1独立区間化を検証する新規テスト1件を追加、全62件成功。`tsc --noEmit`・`eslint`・`next build`すべて成功。
- 実ブラウザでの最終確認はまだ行っていない。

> 次アクション候補：ユーザーによる実ブラウザでの動作確認待ち（社内・外部共有それぞれのガントで「先方チェック待ち」ピンクと「社内バッファ待ち」ティール系の2色が意図通り区別できるか）。

**2026-08-28 追記：ユーザーが動作確認し「OKです」との回答。セッションクローズ**

ユーザーが本番環境で色分け（先方チェック待ち＝ピンク／社内バッファ待ち＝ティール系）を確認し「OKです」との回答。コミット・push済み（`2c656f7`・`2253345`）、Vercel本番環境（GitHub連携の自動デプロイ）へ反映済み。このセッションで着手した3件の要望はすべて対応・確認完了。

**次回セッションへの引き継ぎ事項**：

- スケジュール自動算出ロジックの仕様は`docs/schedule.md`が正（このセッションで新設）。実装（`computeSchedule.ts`）と食い違いが疑われたら、まずこのファイルと突き合わせること。
- 未解消のまま残る既知のギャップ（対応不要、記録のみ）：`master.default_parallel_by_phase`・`projects.parallel_by_phase`のDBカラムが、並行作業人数の概念撤廃後も未使用のまま残置されている（`supabase/migrations/20260803015113_initial_schema.sql`）。実害はないが、将来DBスキーマを整理する際にdropする候補。
- このリポジトリでは、機能や項目をUI・ロジックから撤廃する際、DBカラム自体は破壊的変更を避けて残置し、masterplan.mdに既知のギャップとして記録する運用を2回連続で踏襲している（2026-08-07の「優先度」撤廃、2026-08-28の「並行作業人数」撤廃）。次に何かを撤廃する際も同じ方針でよいか、都度ユーザーに確認するのが無難。
- 直近3コミット（`3ebf28a`→`2c656f7`→`2253345`）でスケジュール周りに継続して手を入れたため、次にスケジュール関連の要望が来た場合はこの3件の経緯（並行作業人数撤廃・週末バー修正・色分け）を先に読んでおくと二度手間を防げる。

> 次アクション候補：特になし。ユーザーからの次の要望・不具合報告待ち。
