...
1. Supabase Personal Access Tokenが失効済みか確認する。
2. ユーザー本人による実ブラウザでの再確認待ち：2校期間対象工程（構成/デザイン/コーディング/テストアップ）で、制作→CB1(ピンク)→2校作業(青)→CB2(ピンク)の順にガントが表示されているか、CB1・CB2それぞれをクリックして個別に手動編集できるか、CB1を編集した際に2校作業・CB2・次工程が自動的に追従するか、「同じグループの他ページにも同様の修正を反映する」チェックボックスで他ページへも同じ日数分ずらして反映されるか。加えて未確認であれば前回分（非稼働日にバーが被らないか、2校期間の日数のマスタ入力・保存、構成工程の同日一斉着手、チャート上部のサマリー、「作業完了」ラベルと黒い共通公開セル、モーダルでの手動編集、縦罫線の連続性、チェックバック・バッファのピンク色、祝日自動取得）も合わせて確認してもらう。
...
8. カレンダー型グリッドのように「横スクロールする固定幅コンテナ」を作る場合、その内側にクリック時に表示するフォーム・モーダル等を置くと、スクロール位置によっては画面外に隠れて見えなくなる。スクロール非依存の要素は必ずスクロールコンテナの外に置くこと（今回の教訓、モーダル化で解消済み）。
9. 「仕様通りのはず」と説明する前に、実際にそのロジックパスが本当にコード上で保証されているか（今回のように、並行レーンの仕組みが別工程の挙動に暗黙的に影響していないか）を再確認してから回答すること（構成工程の同日着手が実際には保証されていなかった教訓）。
10. 期間・日数を扱うUIで「開始日〜終了日」を単純な1本の帯として描画すると、非稼働日を含む場合に誤解を招く（今回の「土日に作業しているように見える」不具合の教訓）。稼働日ベースの区間分割を検討すること。
