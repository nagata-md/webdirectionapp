...
1. Supabase Personal Access Tokenが失効済みか確認する。
2. ユーザー本人による実ブラウザでの再確認待ち：非稼働日にバーが被らなくなっているか、2校期間の日数がマスタ設定で入力・保存できるか、実際のスケジュール計算に2校期間が正しく反映されているか。加えて前回分（構成工程の同日一斉着手、チャート上部のサマリー、「作業完了」ラベルと黒い共通公開セル、モーダルでの手動編集、縦罫線の連続性、チェックバック・バッファのピンク色、祝日自動取得）も未確認であれば合わせて確認してもらう。
