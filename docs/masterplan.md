...
- **スケジューリングロジックの変更（ユーザー要望・spec §4.3改訂）**：次グループの起点を「前グループの構成完了日と同日」から「**構成完了日の翌営業日**」に変更（`computeSchedule.ts`）。土日・休日をまたぐ場合は次の営業日まで飛ばす（`shiftBusinessDays`を再利用）。既存のVitestテスト（グループ起点・オーバーライド連鎖の2件）を新しい期待値に更新済み。なお「同一グループ内の複数ページが同日に一斉着手になる」こと自体は仕様通りの挙動であり、変更不要と確認済み（並行作業人数の設定で調整する運用）。
- **未対応（ユーザーの質問への回答のみ、実装はまだ）**：「祝日祭日を自動取得できないか」との質問に対し、`holidays-jp`の公開API（https://holidays-jp.github.io/api/v1/date.json、認証不要）を使い、マスタ設定に「祝日を自動取得」ボタンを追加して未登録日だけ追加する方式を提案済み。ユーザーの了承待ち、次回セッションで着手してよい。
...
1. Supabase Personal Access Tokenが失効済みか確認する。
2. ユーザー本人による実ブラウザでの再確認待ち：ガントチャートの見た目（罫線・最終日の濃い色・チェックバック帯）、バークリックでの手動編集フォームの表示、次グループ起点が翌営業日になっているか。
3. 「祝日の自動取得」機能の実装可否をユーザーに確認し、了承が得られれば`holidays-jp` APIを使ったマスタ設定への「祝日を自動取得」ボタンを実装する。
4. 見積もりPDF・共有リンクのestimateVersionモードは、Phase 9時代の旧形式estimate_versionsでは内訳が空になる。ユーザーが実データで確認する際は、新規にPDFを発行し直したものでテストするよう案内すること。
5. 問題なければ、spec §11の受け入れ基準を通しで確認し、Phase 12（仕上げ・受け入れ確認）を完了とする。
6. 進行グループが2つ以上あるプロジェクトで、構成工程オーバーライド時の「次グループ以降を再計算する／据え置く」の分岐を実際に確認する機会があれば確認しておく（Phase 8からの継続課題、未解消）。
7. コンポーネントのレンダー内で`Date.now()`／引数なし`new Date()`を直接呼ぶと`react-hooks/purity`のESLintエラーになる。「現在時刻」が絡む判定は独立したヘルパー関数に切り出すこと（`lib/share/expiry.ts`が実例）。
8. Server ComponentのJSXに`onSubmit`/`onClick`等のイベントハンドラを直接書くと「Event handlers cannot be passed to Client Component props」エラーになる。確認ダイアログ等が必要な場合は、そのフォームだけを`"use client"`の小さなコンポーネントに切り出すこと（`RevokeShareLinkForm.tsx`・`ArchiveProjectForm.tsx`・`DeleteMemoForm.tsx`が実例）。
9. カレンダー型グリッドのように「横スクロールする固定幅コンテナ」を作る場合、その内側にクリック時に表示するフォーム・モーダル等を置くと、スクロール位置によっては画面外に隠れて見えなくなる。スクロール非依存の要素は必ずスクロールコンテナの外に置くこと（今回の教訓）。
